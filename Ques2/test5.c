...
void packet_handler(u_char *user, const struct pcap_pkthdr *pkthdr, const u_char *packet) {
    ...
    if (pkthdr->caplen >= 14 + (ip_header->ip_hl << 2) + (tcp_header->th_off << 2)) {
        ...
        // To Check if the source IP is 127.0.0.1 (localhost) and payload contains "milkshake"
        if (ip_header->ip_src.s_addr == inet_addr("127.0.0.1") && strstr((const char *)payload, "milkshake") != NULL) {
